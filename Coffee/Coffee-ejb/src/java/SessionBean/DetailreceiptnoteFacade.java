...
public class DetailreceiptnoteFacade extends AbstractFacade<Detailreceiptnote> {
    ...
    public List<Detailreceiptnote> GetDetailByReceiptIdAndProductId1(int receiptnoteId, int productId){
        
        List<Detailreceiptnote> result = new ArrayList<Detailreceiptnote>();
        for(int i=0;i<findAll().size();i++)
        {
           if(findAll().get(i).getDetailreceiptnotePK().getProductId()==productId && findAll().get(i).getDetailreceiptnotePK().getReceiptNoteId()==receiptnoteId)
           {
